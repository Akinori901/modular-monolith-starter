...
```bash
make verify-cakephp
# 内訳: deptrac（モジュール境界）→ PHPStan → PHPUnit（app + modules）
```

**PHPStan には `--memory-limit=512M` が必須。** 既定の 128M ではワーカーが
クラッシュし、型エラーではなく `PHPStan process crashed` が出て解析が完走しない。
Makefile と CI には設定済みだが、個別に叩くときは自分で付けること。
...
解析対象は `src/` と `modules/` の両方。**ベースライン（`ignoreErrors`）は空で、
これは「完全準拠している」状態を表す。** 新規に書いたコードの型エラーを
ここへ登録して通さないこと。
...
| `--testsuite=modules` | `modules/*/tests/TestCase/` | **公開 API とイベント購読** |

`make verify-cakephp` は両方を実行し、**`|| true` は付いていない**
（落ちればそのまま verify が落ちる）。CI も同じ構成。

**新しいモジュールにテストを足すときは `composer.json` の `autoload-dev` に
`<Name>\Test\` を追加すること。** 無いとテストクラスが解決されず、
「書いたのに実行されない」状態になる。
